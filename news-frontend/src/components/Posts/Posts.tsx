    ...
    <Container>
      <Grid container sx={{ pt: 2 }} spacing={2} justifyContent="space-between">
        <Grid
          ...
          alignItems="center"
          sx={{ mb: 2, flexGrow: 1, flexBasis: '100%' }}
        >
          <Typography variant="h3" component="span">
            Posts
          </Typography>
          <Button to="/add-post" component={NavLink} color="primary" variant="contained" sx={{ mr: 2 }}>
            Add new post
          </Button>
        </Grid>
        {isLoading ? (
          ...
          posts.map((item) => (
            <PostsItem id={item.id} title={item.title} image={item.image} created_at={item.created_at} key={item.id} />
          ))
        )}
      </Grid>
    </Container>
